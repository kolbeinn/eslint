...
/**
 * Fix for missing rule definition
 * @param {Object} options to create missing rule definition fix
 * @param {("disable"|"enable"|"disable-line"|"disable-next-line")} options.type The type of directive comment
 * @param {token} options.commentToken The Comment token
 * @param {string} options.justification The justification of the directive
 * @param {string[]} ruleIds rules in the directive
 * @param {string} missingRuleId The ruleId that should be removed from the directive
 * @returns {Object} A fix command for the missing rule directive
 */
function fixMissingDefinitionDirective(options, ruleIds, missingRuleId) {
    if (ruleIds.length === 1) {
        return ruleFixer.remove(options.commentToken);
    }
    const remainingRuleIds = ruleIds.filter(ruleId => ruleId !== missingRuleId);
    let replacementText = `eslint-${options.type} ${remainingRuleIds.join(" ")}`;

    // TODO: add justification back in
    if (options.commentToken.type === "Line") {
        replacementText = `// ${replacementText}`;
    } else {
        replacementText = `/* ${replacementText} */`;
    }
    return ruleFixer.replaceText(options.commentToken, replacementText);
}
...
function createDisableDirectives(options) {
    ...
    for (const ruleId of directiveRules) {
        ...
        if (ruleId === null || !!ruleMapper(ruleId)) {
            ...
            const problem = createLintingProblem({ ruleId, loc: commentToken.loc });

            problem.fix = fixMissingDefinitionDirective(options, ruleIds, ruleId);
            result.directiveProblems.push(problem);
        }
    }
    return result;
}
